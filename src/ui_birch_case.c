...
static void BirchCase_ReturnFromNamingScreen(void);
...
{   
    if (!gPaletteFade.active)
    {
        ...
        BirchCaseFreeResources();
        DestroyTask(taskId);
        DoNamingScreen(NAMING_SCREEN_NICKNAME, gStringVar2, GetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_SPECIES, NULL), GetMonGender(&gPlayerParty[gSpecialVar_0x8004]), GetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_PERSONALITY, NULL), BirchCase_ReturnFromNamingScreen);
    }
}
...
static u16 BirchCase_GetNextPosition(u16 position, u16 direction)
{
    switch (direction)
    {
    case DPAD_UP:
        if(position <= BALL_TOP_FOURTH)
        {
            if(position < BALL_TOP_THIRD)
                return BALL_BOTTOM_FIRST;
            else
                return BALL_BOTTOM_SECOND;
        }
        else if(position <= BALL_MIDDLE_THIRD)
        {
            if(position == BALL_MIDDLE_FIRST)
                return BALL_TOP_FIRST;
            else if (position == BALL_MIDDLE_SECOND)
                return BALL_TOP_SECOND;
            else
                return BALL_TOP_THIRD;
        }
        else
        {
            return BALL_MIDDLE_SECOND;
        }
    case DPAD_DOWN:
        if(position <= BALL_TOP_FOURTH)
        {
            if(position < BALL_TOP_THIRD)
                return BALL_MIDDLE_FIRST;
            else if(position == BALL_TOP_THIRD)
                return BALL_MIDDLE_SECOND;
            else
                return BALL_MIDDLE_THIRD;
        }
        else if(position <= BALL_MIDDLE_THIRD)
        {
            if(position < BALL_MIDDLE_SECOND)
                return BALL_BOTTOM_FIRST;
            else
                return BALL_BOTTOM_SECOND;
        }
    ...
    case DPAD_RIGHT:
        ...
        {
            if(position == BALL_BOTTOM_SECOND)
                return BALL_BOTTOM_FIRST;
            else
                return position + 1;
        }
    case DPAD_LEFT:
        if(position <= BALL_TOP_FOURTH)
        {
            if(position == BALL_TOP_FIRST)
                return BALL_TOP_FOURTH;
            else
                return position - 1;
        }
        else if(position <= BALL_MIDDLE_THIRD)
...
static void BirchCase_MoveToNextBall(u16 direction)
{
    u16 i;

    for (i = 0; i < ARRAY_COUNT(sStarterChoices); i++)
    {
        sBirchCaseDataPtr->handPosition = BirchCase_GetNextPosition(sBirchCaseDataPtr->handPosition, direction);
        if (sStarterChoices[sBirchCaseDataPtr->handPosition].species != SPECIES_NONE)
            return;
    }
}
...
static void Task_BirchCaseMain(u8 taskId)
{
    ...
    {
        PlaySE(SE_SELECT);
        BirchCase_MoveToNextBall(DPAD_UP);
    ...
    {
        PlaySE(SE_SELECT);
        BirchCase_MoveToNextBall(DPAD_DOWN);
        ChangePositionUpdateSpriteAnims(oldPosition, taskId);
        return;
    }
    ...
    {
        PlaySE(SE_SELECT);
        BirchCase_MoveToNextBall(DPAD_RIGHT);
        ChangePositionUpdateSpriteAnims(oldPosition, taskId);
        return;
    }
    ...
    {
        PlaySE(SE_SELECT);
        BirchCase_MoveToNextBall(DPAD_LEFT);
        ChangePositionUpdateSpriteAnims(oldPosition, taskId);
        return;
    }
    if(JOY_NEW(A_BUTTON))
