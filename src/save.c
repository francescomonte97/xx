...
static u16 CalculateChecksum(void *data, u16 size)
{
    ...
    u32 checksum = 0;
    u32 *d = (u32 *)data;
    const u32 tweak1 = 0xA5A5A5A5;
    const u32 tweak2 = 0x5A5A5A5A;

    for (i = 0; i < (size / 4); i++)
    {
        // Combina il dato con due costanti alternate
        u32 val = d[i] ^ ((i & 1) ? tweak1 : tweak2);

        // Fold parziale: somma e xor con checksum corrente
        checksum += val;
        checksum ^= (checksum << ((i % 5) + 1)) | (checksum >> (32 - ((i % 5) + 1)));
    }

    // Riduzione finale a 16 bit con folding multiplo
